...
# Sumber download:
#   "ssstiktok" = ssstiktok.dev (HD utk video; photo carousel fallback ssstik.io)
#   "ssstik"    = ssstik.io (video & photo carousel, standar)
#   "auto"      = coba ssstiktok.dev dulu, fallback ssstik.io
...
#  DOWNLOAD via ssstik
# =========================
def ssstiktok_hd_resolve(video_url: str):
    """
    Resolve URL TikTok -> MP4 HD via ssstiktok.dev.
    - VIDEO biasa: ambil link 'Download MP4 HD'
    - PHOTO carousel: render slideshow 1080p via s1.tik-cdn.com/api/json/convert
    Return ('url', hd_mp4_url) / None.
    """
    sess = _SCRAPER if _SCRAPER is not None else requests.Session()
    hdr = {"User-Agent": USER_AGENT, "Referer": "https://ssstiktok.dev/",
           "Origin": "https://ssstiktok.dev", "X-Requested-With": "XMLHttpRequest",
           "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}
    try:
        r = sess.post("https://ssstiktok.dev/api/ajaxSearch",
                      data={"q": video_url, "lang": "en"}, headers=hdr, timeout=30)
        if r.status_code != 200:
            return None
        html = r.json().get("data", "")

        # Caption dari response ssstiktok.dev - reliable, ga perlu oEmbed
        # Photo carousel pakai <h3>, video biasa pakai <div class=content>
        cap = ""
        mc = (re.search(r"<h3[^>]*>(.*?)</h3>", html, re.DOTALL)
              or re.search(r'<div[^>]*class=["\']?content["\']?[^>]*>(.*?)</div>', html, re.DOTALL))
        if mc:
        ...
        if 'id="ConvertToVideo"' in html or "data-imageData" in html:
            try:
                k_exp = re.search(r'k_exp\s*=\s*["\']([^"\']+)', html).group(1)
                k_token = re.search(r'k_token\s*=\s*["\']([^"\']+)', html).group(1)
                v_id = re.search(r'value=["\'](\d{15,25})["\']', html).group(1)
                audio = re.search(r'data-audioUrl="([^"]*)"', html).group(1)
                imgdata = re.search(r'data-imageData="([^"]*)"', html).group(1)
            except AttributeError:
                return None
            # PENTING: decode HTML entity (&amp; &#xxx; dll) - kalo ga, base64 rusak di server
            audio = _html.unescape(audio)
            imgdata = _html.unescape(imgdata)
            log_dim("photo carousel → render slideshow 1080p (ssstiktok.dev) ...")
            payload = {"ftype": "mp4", "v_id": v_id, "audioUrl": audio,
                       "audioType": "audio/mp3", "imageUrl": imgdata, "fquality": "1080p",
                       "fname": "tikdl.com", "exp": k_exp, "token": k_token}
            chdr = {"User-Agent": USER_AGENT, "Origin": "https://ssstiktok.dev",
                    "Referer": "https://ssstiktok.dev/",
                    "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}
            # Render async: kadang return "Converting..." -> retry sampai jadi
            for attempt in range(20):
                r2 = sess.post("https://s1.tik-cdn.com/api/json/convert",
                               data=payload, headers=chdr, timeout=180)
                try:
                    j = r2.json()
                except Exception:
                    return None
                ...
                if "convert" in res.lower() or j.get("status") in ("processing", "converting"):
                    ...
                    continue
                return None
            log_dim("render timeout (kelamaan converting)")
            return None
        return None
    except Exception as e:
        log_dim(f"ssstiktok.dev error: {e}")
        return None
...
def download_video(video: dict, outdir: Path) -> Optional[Path]:
    ...
    # Sumber 1: ssstiktok.dev (HD utk video biasa)
    if DOWNLOAD_SOURCE in ("ssstiktok", "auto"):
        result = ssstiktok_hd_resolve(webpage_url)
        if result:
            log_dim("sumber: ssstiktok.dev (HD)")
            referer = "https://ssstiktok.dev/"

    # Sumber 2: ssstik.io (video & photo carousel)
    # - kalo source=ssstik: langsung pakai ini
    # - kalo source=auto/ssstiktok & ssstiktok gagal (mis photo carousel): fallback
    if not result and DOWNLOAD_SOURCE != "ssstiktok":
    ...
    try:
        with requests.get(mp4_url, stream=True, timeout=300,
                          headers={"User-Agent": USER_AGENT, "Referer": referer}) as r:
            r.raise_for_status()
...
def main():
    ...
    args = sys.argv[1:]
    # Flag sumber: --ssstiktok / --ssstik / --auto (boleh di posisi mana aja)
    global DOWNLOAD_SOURCE
    for flag, val in [("--ssstiktok", "ssstiktok"), ("--ssstik", "ssstik"), ("--auto", "auto")]:
        if flag in args:
            DOWNLOAD_SOURCE = val
            args.remove(flag)
    ...
    if args:
        ...
        # Pilih sumber download
        print()
        print(_c("  Pilih sumber download:", C.WHITE + C.BOLD))
        print(f"    {_c('1', C.CYAN + C.BOLD)}  Auto          {_c('(ssstiktok.dev HD dulu, fallback ssstik.io)', C.GRAY)}")
        print(f"    {_c('2', C.CYAN + C.BOLD)}  ssstiktok.dev {_c('(HD 1080p, video & photo carousel)', C.GRAY)}")
        print(f"    {_c('3', C.CYAN + C.BOLD)}  ssstik.io     {_c('(standar, video & photo carousel)', C.GRAY)}")
        s = input(f"  {_c('➜', C.MAGENTA)} sumber [1/2/3] (default 1): ").strip() or "1"
        DOWNLOAD_SOURCE = {"2": "ssstiktok", "3": "ssstik"}.get(s, "auto")
    print()
    hr()
    ...
    log_info(f"sumber download: {_c(DOWNLOAD_SOURCE, C.YELLOW)}")
    hr()
