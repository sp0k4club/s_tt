...
def get_tiktok_videos_from_file(urls_file: str, max_video: int = 0, skip_ids: set = None):
    ...
    # Caption di-fetch nanti pas mau post tiap video (1-1), biar ga kena rate-limit
    print(f"[urls] {len(items)} video baru dari {urls_file}")
    return items
...
def main():
    ...
    for i, v in enumerate(videos, start=1):
        ...
        # Fetch caption asli TikTok via oEmbed (kalo belum ada & ada page URL)
        if not v.get("title") and v.get("_page_url"):
            cap = _fetch_tiktok_caption(v["_page_url"])
            if cap:
                v["title"] = cap
            print(f"[caption] {v.get('title') or '(kosong)'}")
